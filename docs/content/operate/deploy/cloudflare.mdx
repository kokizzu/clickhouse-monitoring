...
|---|---|---|---|
| **Build-time client vars** | CI env / shell before `pnpm run build` | Browser JS (VITE-inlined) | `VITE_AUTH_PROVIDER`, `VITE_CLERK_PUBLISHABLE_KEY` (derived from `CHM_AUTH_PROVIDER` / `CHM_CLERK_PUBLISHABLE_KEY`) |
...
```bash
# In your CI environment / GitHub Actions secrets, before `pnpm run build`:
...
Server-side persistence requires `CHM_FEATURE_CONVERSATION_DB=true` set **at build time** (in CI before `pnpm run build`; the client `VITE_FEATURE_CONVERSATION_DB` derives from it), plus Clerk auth. The backend is then selected at runtime.
