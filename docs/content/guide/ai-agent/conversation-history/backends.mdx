...
All server-side backends require `CHM_FEATURE_CONVERSATION_DB=true` set **before** `pnpm run build`. Its client `VITE_FEATURE_CONVERSATION_DB` is derived from it at build time and baked into the bundle, so it cannot be changed without a rebuild. `CONVERSATION_STORE_BACKEND` is a runtime variable and can be set or changed without rebuilding.
</Callout>
