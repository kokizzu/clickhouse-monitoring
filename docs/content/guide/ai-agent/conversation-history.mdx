...
1. `CHM_FEATURE_CONVERSATION_DB=true` must be set **at build time** (before `pnpm run build`). The client `VITE_FEATURE_CONVERSATION_DB` value is derived from it and baked into the bundle, so it cannot be changed at runtime. It also requires Clerk auth to be active (`CHM_AUTH_PROVIDER=clerk`).
2. A backend must be reachable at runtime.

```bash
# In CI / build environment (before pnpm run build):
